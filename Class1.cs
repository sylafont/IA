...
namespace ProjetIA2022
{
    public class Node2 : GenericNode 
    {
        ...
        public override double CalculeHCost()
        {
            ...
            int differenceX = Math.Abs(x - Form1.xfinal);
            int differenceY = Math.Abs(y - Form1.yfinal);
            int heuristique = differenceX + differenceY;
            
            if(y > Form1.yfinal)
            {
                if(differenceY > differenceX)
                {
                    heuristique = differenceX*50 + (differenceY-differenceX)*50;
                }
                else
                {
                    heuristique = differenceY*50 + (differenceX - differenceY) * 30;
                }
            }
            else
            {
                if (differenceY > differenceX)
                {
                    heuristique = differenceX * 20 + (differenceY - differenceX) * 21;
                }
                else
                {
                    heuristique = differenceY * 20 + (differenceX - differenceY) * 30;
                }
            }
